...
unique_tags = np.array([tag for tag in train_dataset.tag_vocab.keys() if tag not in ["<PAD>", "<UNK>"]])
...
loss_fn = nn.CrossEntropyLoss(ignore_index=train_dataset.tag_vocab["<PAD>"], weight=weights_tensor)
